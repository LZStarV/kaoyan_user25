...
Page({
  ...
  async getCollegeDetail() {
    const [res1, res2] = await Promise.all([
      getCollegetIntro(this.data.id),
      getCollegeInfo(this.data.id),
    ]);

    const infoArray = res1.data.split('\n');
    const infoNodes = infoArray
      .map(
        (info) =>
          `&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${info.replace(/\n/g, '<br>')}`,
      )
  ...
  click_detailType(e) {
    const detail_type_Index = e.currentTarget.dataset.index;
    this.setData({
      detail_type_Index,
      isfull: true,
    });
  ...
  click_subject_type: function (e) {
    const subject_type_Index = e.currentTarget.dataset.index;
    this.setData({
      subject_type_Index,
      sub_subject_type_Index: 0,
      type_of_sub_subjects:
        this.data.type_of_subjects_all[
          this.data.type_of_subjects[subject_type_Index].name
        ],
      'requestSubjectData.fieldId':
        subject_type_Index === 0 ? '' : subject_type_Index,
    });
  },
  //选择专业二级
  click_sub_subject_type: function (e) {
    const sub_subject_type_Index = e.currentTarget.dataset.index;
    console.log(this.data.type_of_sub_subjects[sub_subject_type_Index]);
    this.setData({
      sub_subject_type_Index,
      'requestSubjectData.disciplineId':
        sub_subject_type_Index === 0
          ? ''
          : this.data.type_of_sub_subjects[sub_subject_type_Index].disciplineId,
    });
  },
  //选择学习方式 1.非全日制  2.全日制
  click_way(e) {
    const way = e.currentTarget.dataset.index;
    this.setData({
  ...
  click_like(e) {
    ...
    const newstarJudge = !this.data.starJudge;
    const newShare = [...share];
    if (!this.data.starJudge) newShare[index].like += 1;
    else newShare[index].like -= 1;
    this.setData({
