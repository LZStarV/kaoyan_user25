...
import { getArticleDetail,likeArticle,starArticle} from '../../../api/article'
Page({
  ...
  data: {
    ...
    // 开始计时
    startTimer() {
      if (this.data.isRunning) return; // 如果已经在运行，则不重复开始
      ...
      this.data.intervalId = setInterval(() => {
        ...
        this.setData({time: `${String(hours).padStart(2, "0")}:${String(minutes).padStart(2, "0")}:${String(seconds).padStart(2, "0")}`});
      }, 1000);
    },
  
    // 暂停计时
    pauseTimer() {
      if (!this.data.isRunning) return; // 如果没有在运行，则不操作
      clearInterval(this.data.intervalId);
      this.setData({isRunning: false});
    },
  ...
  click_like()
  {
    const {isLiked,info:{likeCount}}=this.data
  ...
  click_star()
  {
    const {isStared,info:{starCount}}=this.data
