...
Page({
  ...
  data: {
      ...
      bannerList: [{cover:'../../../images/home/swiper_1.png'},
    ...
    async getDataList() {
      try {
        ...
        bannerList.forEach((item) => {
          item.navUrl = item.jumpType == "1" 
            ? '/pages/home/articleDetail/ArticleDetail?id=' + item.postId 
            : funcList[Number(item.funcType)];
        });
        this.setData({
          loading: false,
          bannerList,
  ...
  async getDataList() {
    ...
    bannerList.forEach((item, index, array) => {
      ...
      else if (item.jumpType == "2") navUrl = funcList[Number(item.funcType)];
