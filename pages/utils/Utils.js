...
Page({
  ...
  data: {
    ...
    experiences: [
      ...
      {
        title: "学姐B：心态很重要",
        content: "保持积极心态，不要因为一时的挫折而放弃。相信自己，你一定可以！"
      },
      {
        title: "学长C：多做真题",
        content: "真题是最好的复习资料，多做真题，总结经验，提高答题速度和准确率。"
      }
    ],
    id:'',//物品分类id
  ...
  async getItemDataList() {
    if (this.data.isLoading || this.data.isFinish) return; // Prevent concurrent requests
  ...
  async getAllDataList() {
    const [res1, res2] = await Promise.all([getNavList(), getItemList(this.data.id)]);
    this.setData({
