...
Page({
  ...
  initdata(e) {
    ...
    const cur_day = date.getDate(); // 获取当前日期
  ...
  calculateEmptyGrids: function (year, month) {
    ...
    const firstDayOfWeek = this.getFirstDayOfWeek(year, month) || 7;
  ...
  onGetSignUp: function (e) {
    ...
    self.setData(
      {
        signUp: [
          { date: '2024-9-1', Score: '6' },
          { date: '2024-9-2', Score: '6' },
          { date: '2024-9-3', Score: '6' },
          { date: '2024-9-5', Score: '6' },
          { date: '2024-9-9', Score: '6' },
          { date: '2024-9-8', Score: '6' },
        ],
      },
      function () {
        self.onJudgeSign();
  ...
  onJudgeSign: function () {
    const that = this;
    const signs = that.data.signUp;
    const days = that.data.days;
    const cur_month = that.data.cur_month; //选中月
    const cur_year = that.data.cur_year; //选中年
    for (let i = 0; i < signs.length; i++) {
      const current = new Date(signs[i].date);
      const year = current.getFullYear();
      const month = current.getMonth() + 1;
      let day = current.getDate();
      day = parseInt(day);
      for (let j = 0; j < days.length; j++) {
        if (year === cur_year && month === cur_month && days[j].date === day) {
          //已签到&&年月日相同
          days[j].isSign = true;
